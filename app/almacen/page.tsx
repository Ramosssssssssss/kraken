...
export default function AlmacenesPage() {
  ...
  const filteredAlmacenes = useMemo(() => {
    ...
    return filtered.filter(almacen =>
      ...
      almacen.ALMACEN_ID.toString().includes(searchQuery)
    );
  ...
  const handleAddAlmacen = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Agrega el nuevo almacén al estado local para actualizar la UI inmediatamente
        const newAlmacen: Almacen = {
          ALMACEN_ID: data.ALMACEN_ID || Date.now(), // Usa el ID de la respuesta o uno temporal
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredAlmacenes.map((almacen, index) => (
              <div key={almacen.ALMACEN_ID} onClick={() => handleSelectAlmacen(almacen)} className="group relative bg-neutral-900/40 backdrop-blur-xl border border-neutral-800/50 rounded-2xl p-8 cursor-pointer transition-all duration-500 hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-2 hover:border-neutral-700/50 overflow-hidden" style={{ animationDelay: `${index * 100}ms` }}>
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <span className="text-xs text-neutral-500 bg-neutral-800/80 backdrop-blur-sm px-3 py-1.5 rounded-full font-mono">#{almacen.ALMACEN_ID}</span>
                  </div>
